...
```sql
-- retry queue backlog (ready/pending/dead)
select
  count(*) filter (where status in ('pending','failed') and next_retry_at <= now()) as ready,
  count(*) filter (where status in ('pending','failed') and next_retry_at > now()) as pending,
  count(*) filter (where status = 'dead') as dead
from telemetry_delivery_queue;
```

```sql
-- retry queue destination별 dead ratio
select
  destination,
  count(*) filter (where status = 'dead') as dead_jobs,
  count(*) as total_jobs,
  count(*) filter (where status = 'dead')::float / nullif(count(*), 0) as dead_ratio
from telemetry_delivery_queue
group by destination
