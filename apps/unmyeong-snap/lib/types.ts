...
export type TelemetryRetryStatus = "pending" | "processing" | "sent" | "failed" | "dead";
export type TelemetryRetryDestination = "amplitude" | "meta";

export interface TelemetryRetryJob {
  jobId: number;
  eventId: string;
  destination: TelemetryRetryDestination;
  status: TelemetryRetryStatus;
  attemptCount: number;
  nextRetryAt: string;
  lastError?: string;
  lockedAt?: string;
